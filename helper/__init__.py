...
def resize(image):
    ...
    if sc>1:
        x = width-height
        image = cv2.copyMakeBorder(image,x/2,x/2,0,0,cv2.BORDER_CONSTANT,value=[0,0,0])
    if sc<1:
        x = height-width
        image = cv2.copyMakeBorder(image,0,0,x/2,x/2,cv2.BORDER_CONSTANT,value=[0,0,0])
...
def list_item(input_path):
    for item in os.listdir(input_path):
        if item in NAME_LIST:
            ...
            for data_item in os.listdir(path):
                if data_item.endswith('.png') or data_item.endswith('.jpg'):
                    path_item = os.path.join(path, data_item)
                    yield path_item
